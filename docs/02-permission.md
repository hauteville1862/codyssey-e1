...
| `w` | 쓰기(write) | 2 (2¹) |
| `x` | 실행/진입(execute) | 1 (2⁰) |
| — | 서로 다른 2의 거듭제곱이라 합이 겹치지 않아, rwx 조합마다 0~7 중 유일한 숫자로 표현 가능 | — |
...
### 2-1. 권한 확인
`ls -ld 대상`: 파일과 디렉토리의 권한을 `-rwxr-xr-x` 형식(맨 앞은 파일 종류, 이후 3자리씩 소유자·그룹·기타)으로 출력하는 명령어. 
- `-l`은 상세 정보, `-d`는 디렉토리 자체 정보(내부 목록 아님)를 의미함
...
$ chmod 755 folder/test.sh
```
> 환경B(Windows+git bash)는 실행 비트가 강제되지 않아 400 적용 후에도 `x`가 남아 `-r-xr-xr-x`로 표시됨(쓰기 차단만 정상 반영). 리눅스/macOS(환경A)에서는 `r--------`로 표시됨
