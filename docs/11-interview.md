...
기준은 하나다. **"이 명령을 실행하는 위치가 항상 같다고 보장할 수 있는가?"** 보장되면 상대 경로를 쓰고, 보장되지 않으면 절대 경로를 쓴다.

- **절대 경로**: `/`(리눅스·macOS) 또는 `C:/`(Windows)로 시작하며 파일시스템 최상위(루트)를 기준으로 하므로, 현재 위치가 어디든 항상 같은 대상을 가리킨다.
- **상대 경로**: `note.txt`, `./site`, `../docs`처럼 `/`로 시작하지 않으며 **현재 작업 디렉토리(`pwd`)**를 기준으로 해석된다. 

## 호스트 포트가 이미 사용 중이라 포트 매핑이 실패한다면, 어떤 순서로 원인을 진단할지?

1. **에러 메시지로 원인을 확정한다.** `Bind for 0.0.0.0:8080 failed: port is already allocated`처럼 Docker가 충돌을 명시적으로 알려주므로 추측 전에 문구부터 읽는다. 이때 문제가 되는 건 호스트 포트뿐이고, 컨테이너 포트(예: 80)는 컨테이너마다 독립된 네임스페이스라 겹쳐도 무관하다는 점을 먼저 구분한다.
